...
| R-002 | Pinyin dependency/version drift changes public URL | Medium | Critical | Content / 1、5、6 | Pin version; run Phase 0 fixture against ingestion and App Router | OPEN |
...
| R-004 | 7 existing Wiki Alias are mistaken for permission to create a broad redirect map | Medium | High: SEO/analytics drift | Content / 5、6、18 | Explicit allowlist; per-alias test; Owner approval for additions | OPEN |
...
| R-006 | Explicit Blog paths containing `!`/`_` are normalized | Medium | High | Content/Web / 5、6 | Explicit `path` priority fixture; exact URL assertion | OPEN |
| R-007 | No stable author ID makes rename/move continuity ambiguous | High | High: duplicate/lost history | DB/Content / 3、5 | Source-commit diff + hash; transaction; manual decision when unsafe | OPEN |
| R-008 | Delete is implicit in static builds and stale DB rows remain public | High | High | Content / 5、18 | Snapshot reconciliation and delete integration tests | OPEN |
...
| R-017 | Old Build-time locale generation or translation scripts cause paid calls during publish | Medium | Critical cost | Translation / 5、8、9、15 | Zero provider call tests; explicit job + server budget only | OPEN |
| R-018 | Ignored generated locale Markdown is mistaken for Canonical content | High | High | Content/i18n / 5、7、8 | GitHub zh-CN-only directionality test; generated data stays Runtime-only | OPEN |
