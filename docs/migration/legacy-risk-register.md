...
| R-005 | Unprefixed zh-CN URLs are lost when V2 adopts locale-prefixed routing | High | Critical | Web / 5、6、18 | Phase 6 unprefixed and `/zh-cn` real routes PASS；Phase 18 final route audit | CONTROLLED |
| R-006 | Explicit Blog paths containing `!`/`_` are normalized | Medium | High | Content/Web / 5、6 | Phase 5 fixtures + Phase 6 four exact real App Router responses PASS | CONTROLLED |
...
| R-010 | 311 static GitBook HTML routes and 647 companion Markdown/asset files are silently dropped | High | High | Assets / 6、11、18 | Phase 6 byte-identical 958-file archive and all 311 HTML response PASS；Phase 18 delta pending | CONTROLLED |
| R-011 | Thousands of CDN references include Unicode and historic keys | Medium | High | Storage / 6、11、18 | Phase 6 validated local asset/Unicode render PASS；Phase 11 provider-neutral S3 contract + AList evidence and Phase 18 audit pending | CONTROLLED |
| R-012 | External audio/playlist/suggestion/friend endpoints fail or change | High | Medium | Web / 6、11、16、18 | Phase 16 server has no external API dependency; HTTPS links/browser failure isolation + first-party S3 probe；Phase 18 final current-link/feature delta | CONTROLLED |
| R-013 | Public Umami share access and per-page Traffic UI disclose more than intended | Medium | High | Security / 16、18 | Owner approved the exact production tracker/Website ID on 2026-09-07；domain allowlist prevents local/staging collection；CSP permits only the Umami origin；Share ID is public but resolved token/raw rows stay server-only；public UI exposes aggregate path metrics only | CONTROLLED |
...
| R-015 | Personal edit token/localStorage design is copied without a new Trust-boundary review | Medium | Critical | Security / 4、16 | Phase 16 review PASS：Control-only CAS write、Zod、Capability、signing/OIDC、Replay/Rate/Audit；no cookie/localStorage edit token；rotation documented | CONTROLLED |
...
| R-017 | Old Build-time locale generation or translation scripts cause paid calls during publish | Medium | Critical cost | Translation / 5、8、9、15 | Phase 5/7 negative import/fallback + Phase 8 incremental TM + Phase 9 budget/provider boundary + Phase 15 manual-only workflow PASS | CONTROLLED |
...
| R-022 | Analytics-sensitive Alias/Locale paths split or lose historical traffic continuity | Medium | High | Web/Observability / 6、7、16、18 | Phase 7 exact prefixed/same-logical-path PASS；Phase 16 aggregate-only privacy contract；historical aggregate/final audit remains Phase 18 | CONTROLLED |
...
| R-025 | V2 Metadata/Canonical 处理错误地合并 unprefixed、Locale 或 Alias Route，造成重复索引或内容身份漂移 | Medium | High: SEO/URL continuity regression | Web/i18n / 6、7、16、18 | Phase 7 locale-aware title/description + exact route PASS；Phase 16 privacy/route review PASS；final live indexability/social audit remains Phase 18 | CONTROLLED |
