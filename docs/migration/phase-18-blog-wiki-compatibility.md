...
| Wiki list | document-root grouping, index/overview entry, four-digit hierarchical order, continuous `1`/`1.1` numbering, explicit expand/collapse, PostgreSQL/PGroonga title + heading + body search and document traffic aggregation; the homepage takes at most five top-level documents rather than five child chapters | ported algorithm fixture; five-document selection fixture; Public E2E |
| Blog article | stable heading anchors; normalized hierarchical numbering shared by the rendered body and TOC across every CommonMark heading depth `h1`–`h6`, beginning at the highest level actually present and compressing skipped levels; the blue numbered heading itself is the mouse/keyboard anchor target instead of requiring a small `#`; a widened bounded reading column with the desktop TOC moved to the outer edge, mobile TOC drawer, active TOC location, reading progress, code copy, image zoom, reading time, previous/next and aggregate traffic | Markdown hierarchy unit fixture; component unit test; wide-desktop and mobile Public E2E |
| Wiki article | a widened bounded center column with desktop document sidebar and TOC moved toward the viewport edges; centered mobile document/TOC modal with backdrop/Escape/navigation close, lower-left floating chapter/TOC controls, all Blog reader interactions and hierarchy-aware previous/next; the visually dominant print control and dedicated print stylesheet were removed by Owner decision on 2026-09-07 | component unit test; desktop/mobile Public E2E; Owner acceptance feedback |
| Markdown/resource | sanitized GFM Markdown and Shiki remain authoritative; fenced-code language metadata is preserved and rendered in the Legacy-confirmed dark navy code frame with a separate language toolbar and compact copy action in every site theme; Shiki covers the canonical corpus languages (`bash`, C/C++, CMake, Dart, Dockerfile, HTML/XML, JavaScript/TypeScript, JSON/JSONC, Lua, Markdown, PowerShell, Python, SQL, YAML and text/config/diff variants) and validates known historical case/typo aliases before a safe text fallback; the Simplified-Chinese development fixture exercises emphasis, strikethrough, quotes, separators, nested/ordered/task lists, wide tables, long code, links, images and `h1`–`h6`; HTTP(S) external links are isolated; `/docs/**` links remain normal archive navigation; common attachment links are marked; images stay responsive/lazy/async and zoomable, with any click inside the preview closing it; wide tables/code scroll within their own bounded regions while normal long text/links wrap, so narrow viewports do not acquire page-level horizontal overflow | canonical language-corpus unit fixture; Markdown/component fixtures; narrow-viewport Public E2E |
...
The 2026-09-08 code-frame refinement was checked against the exact Legacy Wiki and Blog enhancement implementations at `app/pages/wiki/[...slug].vue` and `app/pages/blog/[...slug].vue`. Only the durable behavior and visual relationship were retained: language label, separate toolbar, compact copy state and bounded horizontal scrolling. The Vue/CSS architecture was not copied.

The same targeted Legacy check confirmed that the Wiki index is organized around top-level documents with expandable chapters and that article navigation occupies the outer desktop rails. V2 retains those information relationships while using the current blue visual system. The exact Legacy favicon bytes were restored at the Next.js app-icon boundary; no legacy component or framework code was copied.
