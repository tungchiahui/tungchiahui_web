...
Phase 14 以 Additive Version 5 Migration 为 Deployment Runtime 增加 Current/Last Digest、Pending Slot/SHA/Digest Cutover Intent、Cutover Timestamp 与当时使用的 Stabilization Deadline。`control-api` 的 Deployment/Rollback Endpoint 与 `./site` 只创建/读取相同 SQLite Operation；既有 `deploy-agent` 分别 Filter Claim Deployment 与 Recovery 类型，并调用唯一 Shared Engine。Deployment Lease 到期保留精确 Persisted Phase，新的 Fencing Token 根据 Pending Intent 与实际 OpenResty Slot 对账，安全 Resume。`productionOperations` 只有在该 Engine 和 Production-like Gate 交付后才为 true。ADR 0020 后续取消固定时间发布拦截，新 Cutover 将兼容列写为 `NULL`；其余状态机、互斥和审计边界不变。
...
`control-api` 与 `deploy-agent` 通过 setgid/最小组写权限共享同一个 Host-local Store；各自使用 restrictive umask，不获得彼此的业务 Credential。SQLite Snapshot 执行 WAL Checkpoint + `VACUUM INTO`，验证 Schema/Integrity/Environment/Active-Previous SHA/Audit Digest，经 age 加密并复制到现有 AList Bucket 的 `backups/control-state/` 与 R2 Off-site 相同 Prefix。两端 read-back-verified `latest.json` 允许在本地 SQLite 全损时优先从 AList、失败时从 R2 发现最新 Artifact。显式 Break-glass 仅替换到达路径，仍向同一 Store 写 Audit/Operation 并由同一 Agent/Engine 执行。
