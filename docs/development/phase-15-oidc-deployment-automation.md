...
Phase 15 binds GitHub Actions to the Phase 14 shared deployment engine without creating a second CI deployment implementation. The repository defines the normal `main` release path, immutable image publication, reusable content-only trigger and manual translation trigger. Later ADR 0022 consolidated Quality and Deploy into `release.yml`; ADR 0023 added service-digest binding and fail-closed result checks.
...
`.github/workflows/release.yml` accepts only `main` Push and explicit manual specified-SHA dispatch. It resolves a full SHA that must be an ancestor of current `main`, then runs Biome/source/workflow policy, Drizzle consistency, TypeScript, Unit, Production-foundation/Recovery, Integration/E2E, Migration, Renovate validation and Production build. One global non-cancelling concurrency group prevents overlapping Production cutovers; an automatic Push must still equal current `origin/main` immediately before deployment.

The Build Job receives only `contents: read` and `packages: write`. It builds and publishes Web, Service, Recovery and PostgreSQL images under the full SHA, resolves all four digests, and embeds the exact Service Digest in the Web Image Label. The Deployment Job checks out the reviewed control client at `github.workflow_sha`, rather than allowing a specified old application SHA to roll back the control protocol. It receives `contents: read` and `id-token: write`, enters the protected `production` Environment, installs the locked client and runs:
...
A final always-running Result Job requires both Build and Deploy to succeed. If deployment is disabled or skipped, the Workflow intentionally does not report a complete successful release.
...
The deploy-agent alone receives the approved image repository and package-read identity from the service-scoped Host configuration. GitHub Actions never receives that pull identity, the Docker socket, a host login, a Production database credential or an AI-provider credential.

For deployment, the Docker adapter resolves only `<approved-repository>@sha256:<manifest-digest>`, pulls through the Docker Engine API when absent, verifies the exact `RepoDigest`, and requires OCI `org.opencontainers.image.revision` to equal the requested full Git SHA. It then reads the bound Service Digest from that validated Web Image and selects the migration bundle only as `<approved-repository>-service@sha256:<digest>`. The candidate container records the manifest digest separately from Docker's local configuration ID. Explicit rollback validates and switches to the already-running retained container and does not pull or rebuild an image.
...
`check:workflows` parses the workflows and fails if triggers, Quality commands, protected Environment, concurrency, OIDC/package permissions, immutable repositories/digest binding, final release result, shared CLI calls or credential exclusions drift. It validates critical Release Job structure instead of relying only on text fragments. Every third-party Action reference must be a full commit digest. This check runs locally and inside the Quality Gate.
...
Phase 15 adds no PostgreSQL or control-state migration. Application rollback remains the Phase 14 retained-slot, no-rebuild path. Registry credentials can be rotated independently by updating the Host-local `.env` and reconciling only the reviewed independent-service unit; no image or workflow secret contains the origin pull token. If registry pull, digest binding or SHA-label validation fails, the operation fails before candidate mutation and active traffic remains unchanged. Recovery/restore still uses the Phase 13 PostgreSQL-independent path and does not depend on GitHub Actions or the image registry.
